...
class _DatePickerDialogState extends State<DatePickerDialog> with RestorationMixin {
  ...
  Widget build(BuildContext context) {
    ...
    final TextStyle? dateStyle = orientation == Orientation.landscape
        ? context.titleLarge?.copyWith(color: onPrimarySurface)
        : context.titleLarge?.copyWith(color: onPrimarySurface);
...
class _DatePickerHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final Color primarySurfaceColor = isDark ? colorScheme.onSurface : colorScheme.surface;
    final Color onPrimarySurfaceColor = isDark ? colorScheme.surface : colorScheme.onSurface;
    ...
    switch (orientation) {
      case Orientation.portrait:
        return SizedBox(
          ...
          child: Material(
            shape: Border(
                bottom: BorderSide(width: 1,color: context.colorScheme.outline.withOpacity(0.7),)
            ),
...
/// Shows a full screens modal dialog containing a Material Design date range
/// picker.
///
...
///
Future<DateTime?> showDateSinglePicker({
  required BuildContext context,
  ...
  Widget dialog = DateRangePickerDialog(
    ...
    initialEntryMode: initialEntryMode,
    helpText: helpText,
    // helpText: helpText ?? StringApp.of(context).flight_v2_search_select_date.toUpperCase(),
    dateSelectedHintText: dateSelectedHintText,
    cancelText: cancelText,
    confirmText: confirmText,
    saveText: saveText,
    errorFormatText: errorFormatText,
...
String _formatRangeEndDate(MaterialLocalizations localizations, DateTime? startDate,
...
class DateRangePickerDialog extends StatefulWidget {
  ...
  final String? confirmText;
  final String? dateSelectedHintText;
...
class _DateRangePickerDialogState extends State<DateRangePickerDialog>
  ...
  void _handleOkInput() {
    ...
    Navigator.pop(context, _hasSelectedDateRange ? _selectedStart.value! : null);
  }
  ...
  void _handleOk() {
    ...
    final DateTime? selectedRange = _hasSelectedDateRange ? _selectedStart.value! : null;
  ...
  bool get _hasSelectedDateRange => _selectedStart.value != null;
  ...
  Widget build(BuildContext context) {
    ...
    final Color onPrimarySurface =
    colors.brightness == Brightness.light ? colors.onPrimary : colors.onSurface;
    ...
    final bool showEntryModeButton = _entryMode.value == DatePickerEntryMode.calendar ||
        _entryMode.value == DatePickerEntryMode.input;
    switch (_entryMode.value) {
      case DatePickerEntryMode.calendar:
      case DatePickerEntryMode.calendarOnly:
                  ...
                  child: Container(
                    ...
                    child: Column(
                      children: <Widget>[
                        const Spacer(),
                        InputDatePickerFormField(
                          initialDate: _selectedStart.value,
                          firstDate: widget.firstDate,
                          lastDate: widget.lastDate,
                          onDateSubmitted: _handleStartDateChanged,
                          onDateSaved: _handleStartDateChanged,
                          errorFormatText: widget.errorFormatText,
                          errorInvalidText: widget.errorInvalidText,
                          fieldHintText: widget.fieldStartHintText ??
                              "(haloFlutterApp.languageCode == vi ? CommonTransform.format_dd_MM_yyyy() : localizations.dateHelpText)",
                          fieldLabelText: widget.fieldStartLabelText,
                          autofocus: true,
                        ),
                        const Spacer(),
                      ],
...
class _CalendarRangePickerDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    colorScheme.brightness == Brightness.light ? context.colorScheme.onBackground : colorScheme.onBackground;
    final Color headerForegroundBtn =
    colorScheme.brightness == Brightness.light ? context.colorScheme.primary : colorScheme.onSurface;
    final Color headerDisabledForeground = headerForeground.withOpacity(0.38);
    final String startDateText =
    localizations.formatMediumDate(selectedStartDate ?? DateTime.now());
    final TextStyle? headlineStyle = context.titleLarge;
    final TextStyle? startDateStyle = context.titleLarge?.regular.apply(
    ...
    final TextStyle saveButtonStyle = textTheme.button!.apply(
      color: onConfirm != null ? headerForegroundBtn : headerDisabledForeground,
    );
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          actions: <Widget>[
            ...
            TextButton(
              onPressed: onConfirm,
              child: Text(Str.of(context).common_save, style: saveButtonStyle),
            ),
...
class _CalendarDateRangePickerState extends State<_CalendarDateRangePicker> {
  ...
  void _handleDateSelected(DateTime selectedDate) {
    ...
    widget.onStartDateChanged?.call(selectedDate);
  }
  ...
  Widget _buildMonthItem(BuildContext context, int index, bool beforeInitialMonth) {
    ...
    return _MonthItem(
      ...
      onChanged: widget.isSingle ? _handleDateSelected : _updateSelection,
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: <Widget>[
        _DayHeaders(),
        if (_showWeekBottomDivider)  Divider(height: 1,color: context.colorScheme.outline.withOpacity(0.7),),
        Expanded(
...
class _DayHeaders extends StatelessWidget {
  ...
  List<Widget> _getDayHeaders(
      BuildContext context, TextStyle? headerStyle, MaterialLocalizations localizations) {
  ...
  Widget build(BuildContext context) {
    ...
    final TextStyle? textStyle = context.bodyMedium?.normal;
    final MaterialLocalizations localizations = MaterialLocalizations.of(context);
    final List<Widget> labels = _getDayHeaders(context, textStyle, localizations);
...
class _MonthItemState extends State<_MonthItem> {
  ...
  Widget _buildDayItem(
      BuildContext context, DateTime dayToBuild, int firstDayOffset, int daysInMonth) {
    final ThemeData theme = Theme.of(context);
    final ColorScheme colorScheme = theme.colorScheme;
    final TextTheme textTheme = theme.textTheme;
    final MaterialLocalizations localizations = MaterialLocalizations.of(context);
    ...
    final bool isDisabled =
        dayToBuild.isAfter(widget.lastDate) || dayToBuild.isBefore(widget.firstDate);

    BoxDecoration? decoration;
    TextStyle? itemStyle = context.bodyMedium;
    ...
    if (isSelectedDayStart || isSelectedDayEnd) {
    ...
    } else if (isInRange && widget.isSingle == false) {
      ...
      // border.
      itemStyle = textTheme.bodyText2?.apply(color: context.colorScheme.shadow);
      decoration = BoxDecoration(
    ...
    Widget dayWidget = Container(
      ...
      child: Center(
        child: Semantics(
          label: semanticLabel,
          selected: widget.isSingle == true
              ? isSelectedDayStart
              : (isSelectedDayStart || isSelectedDayEnd),
    ...
    if (widget.isSingle == false && highlightPainter != null) {
          ...
          child: ExcludeSemantics(
            child: Text(
              localizations.formatMonthYear(widget.displayedMonth) ??
                  localizations.formatMonthYear(widget.displayedMonth),
              style: context.bodyMedium?.medium.body,
            ),
          ),
        ),
        Container(
...
class _InputDateRangePickerDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    colorScheme.brightness == Brightness.light ? colorScheme.onSurface : colorScheme.surface;
    final TextStyle? dateStyle = orientation == Orientation.landscape
        ? context.titleLarge?.apply(color: onPrimarySurfaceColor)
        : context.titleLarge?.apply(color: onPrimarySurfaceColor);
    ...
    final Widget header = _DatePickerHeader(
      ...
      titleText: helpText ?? localizations.dateRangePickerHelpText,
