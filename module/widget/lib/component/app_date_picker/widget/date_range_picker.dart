...
Future<DateTimeRange?> showDateRangePicker({
  ...
  String? dateEndSelectedHintText,
...
String _formatRangeStartDate(
  ...
  return startDate == null
      ? hintStartDate ?? localizations.dateRangeStartLabel
...
String _formatRangeEndDate(MaterialLocalizations localizations, DateTime? startDate,
    ...
    {String? hintEndDate}) {
  return endDate == null
      ? hintEndDate ?? localizations.dateRangeEndLabel
...
class DateRangePickerDialog extends StatefulWidget {
  ...
  const DateRangePickerDialog({
    ...
    this.restorationId,
    this.isSameDays = true,
    this.dateStartSelectedHintText,
    this.dateEndSelectedHintText,
  }) : super(key: key);
  ...
  final String? dateStartSelectedHintText;
  final String? dateEndSelectedHintText;
    ...
    switch (_entryMode.value) {
      ...
      case DatePickerEntryMode.calendarOnly:
        contents = _CalendarRangePickerDialog(
          ...
          hintStartText: widget.dateStartSelectedHintText,
          hintEndText: widget.dateEndSelectedHintText,
          firstDate: widget.firstDate,
          lastDate: widget.lastDate,
          currentDate: widget.currentDate,
...
class _CalendarRangePickerDialog extends StatelessWidget {
  ...
  final String confirmText;
  final String helpText;
  final String? hintStartText;
  final String? hintEndText;
  final Widget? entryModeButton;
  final bool isSameDays;
  ...
  Widget build(BuildContext context) {
    ...
    final Color headerForegroundBtn = colorScheme.brightness == Brightness.light
        ? context.colorScheme.primary
        : colorScheme.onSurface;
    final Color headerDisabledForeground = headerForeground.withOpacity(0.38);
    final String startDateText = _formatRangeStartDate(
        localizations, selectedStartDate, selectedEndDate,
        hintStartDate: hintStartText);
    final String endDateText = _formatRangeEndDate(
        localizations, selectedStartDate, selectedEndDate, DateTime.now(),
        hintEndDate: hintEndText);
    final TextStyle? headlineStyle = textTheme.headline5;
    final TextStyle? startDateStyle = context.titleLarge?.regular
        .apply(color: selectedStartDate != null ? headerForeground : headerDisabledForeground);
    final TextStyle? endDateStyle = context.titleLarge?.regular.apply(
      color: selectedEndDate != null ? headerForeground : headerDisabledForeground,
    );
    final TextStyle? saveButtonStyle = context.bodyMedium?.medium.apply(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          actions: <Widget>[
            ...
            TextButton(
              onPressed: onConfirm,
              child: Text("Save", style: saveButtonStyle),
            ),
        ...
        // bottomNavigationBar: _buildBottomNav(context, onConfirm, saveButtonStyle),
      ),
    );
  }
}
        ...
        if (widget.isSameDays) {
          ...
          if (_startDate != date) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text(MaterialLocalizations.of(context).invalidDateRangeLabel)));
          }
        }
      } else {
        _startDate = date;
...
class _MonthItemState extends State<_MonthItem> {
  ...
  Widget _buildDayItem(
    ...
    TextStyle? itemStyle = context.bodyMedium?.regular.normal;
    ...
    if (isSelectedDayStart || isSelectedDayEnd) {
      ...
      itemStyle = context.bodyMedium?.apply(color: colorScheme.onPrimary);
      decoration = BoxDecoration(
      ...
      itemStyle = context.bodyMedium?.apply(color: colorScheme.onSurface.withOpacity(0.38));
    } else if (DateUtils.isSameDay(widget.currentDate, dayToBuild)) {
      // The current day gets a different text color and a circle stroke
      // border.
      itemStyle = context.bodyMedium?.apply(color: context.colorScheme.shadow);
      decoration = BoxDecoration(
    ...
    return Column(
      children: <Widget>[
        Container(
          ...
          child: ExcludeSemantics(
            child: Text(
              // localizations.formatMonthYear(widget.displayedMonth).capitalize() ??
...
class _InputDateRangePickerDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final Color onPrimarySurfaceColor =
        colorScheme.brightness == Brightness.light ? colorScheme.onSurface : colorScheme.surface;
    final TextStyle? dateStyle = orientation == Orientation.landscape
        ? context.titleLarge?.copyWith(color: onPrimarySurfaceColor)
        : context.titleLarge?.copyWith(color: onPrimarySurfaceColor);
        ...
        Expanded(
          child: TextField(
            ...
            decoration: InputDecoration(
              ...
              //     (haloFlutterApp.languageCode == "vi"
              //         ? CommonTransform.format_dd_MM_yyyy()
