...
class AppDateTimeException implements Exception {
  final String _cause;
  AppDateTimeException(this._cause);
...
class AppDateTime {
  ...
  AppDateTime([var input, String? pattern]) {
    _initializeDateTime(input, pattern);
    _initializeLocale();
  }

  AppDateTime.unixFromSecondsSinceEpoch(int timestamp) {
    _unix(timestamp * 1000);
  }

  AppDateTime.unixFromMillisecondsSinceEpoch(int timestamp) {
    _unix(timestamp);
  }
  ...
  DateTime _parseString(String input, String? pattern) {
    if (pattern != null) {
      return DateFormat(replacePatternInput(pattern)).parse(replaceParseInput(input));
    } else if (matchHyphenStringDateTime(input)) {
      return DateFormat('yyyy-MM-dd').parse(input);
    } else if (matchUtcStringDateTime(input)) {
      return DateFormat('yyyy-MM-ddThh:mm:ss+hh:mm:ss.SSSz').parse(input);
...
//  MANIPULATE
  ...
  AppDateTime endOf(Units units) {
    switch (units) {
      ...
      case Units.second:
        _dateTime = DateTime(_dateTime.year, _dateTime.month, _dateTime.day, _dateTime.hour,
            _dateTime.minute, _dateTime.second, 999);
        break;
      case Units.minute:
        _dateTime = DateTime(_dateTime.year, _dateTime.month, _dateTime.day, _dateTime.hour,
            _dateTime.minute, 59, 999);
        break;
      case Units.hour:
        _dateTime =
            DateTime(_dateTime.year, _dateTime.month, _dateTime.day, _dateTime.hour, 59, 59, 999);
        break;
      case Units.day:
  ...
  DateTime _addMonths(DateTime from, int months) {
    ...
    if (from.isUtc) {
      return DateTime.utc(newYear, newMonth, newDay, from.hour, from.minute, from.second,
...
//  DISPLAY
  String format([String? pattern]) {
    ...
    return DateFormat(newPattern, _defaultLocale.code).format(_dateTime);
  }

  String yMdHHmmss_separator([String separator = ' ']) {
    return DateFormat.yMd(_defaultLocale.code)
  ...
  String yyyyMMdd_HHmmss_separator([String separator = ' ']) {
    return DateFormat.yMd(_defaultLocale.code)
        .addPattern('', separator)
        .add_Hms()
        .format(_dateTime)
  ...
  String yyyyMMdd_jm_separator([String separator = ' ']) {
    return DateFormat.yMd(_defaultLocale.code)
        .addPattern('', separator)
        .add_jm()
        .format(_dateTime)
  ...
  String yyyyMMdd_jms_separator([String separator = ' ']) {
    return DateFormat.yMd(_defaultLocale.code).add_jms().format(_dateTime).toString();
  }

  String get yyyyMMdd => DateFormat.yMd(_defaultLocale.code).format(_dateTime).toString();

  String get yyyyMMdd_HHmm => DateFormat.yMd(_defaultLocale.code).add_Hm().format(_dateTime);

  String get yyyyMMdd_HHmmss => DateFormat.yMd(_defaultLocale.code).add_Hms().format(_dateTime);

  String get yyyyMMdd_Jm => DateFormat.yMd(_defaultLocale.code).add_jm().format(_dateTime);

  String get yyyyMMdd_Jms => DateFormat.yMd(_defaultLocale.code).add_jms().format(_dateTime);

  String get yMdjm => DateFormat.yMd(_defaultLocale.code).add_jms().format(_dateTime);

  String get E => DateFormat.E(_defaultLocale.code).format(_dateTime);

  String get EEEE => DateFormat.EEEE(_defaultLocale.code).format(_dateTime);

  String get LLL => DateFormat.LLL(_defaultLocale.code).format(_dateTime);

  String get LLLL => DateFormat.LLLL(_defaultLocale.code).format(_dateTime);

  String get Md => DateFormat.Md(_defaultLocale.code).format(_dateTime);

  String get MEd => DateFormat.MEd(_defaultLocale.code).format(_dateTime);

  String get MMM => DateFormat.MMM(_defaultLocale.code).format(_dateTime);

  String get MMMd => DateFormat.MMMd(_defaultLocale.code).format(_dateTime);

  String get MMMEd => DateFormat.MMMEd(_defaultLocale.code).format(_dateTime);

  String get MMMM => DateFormat.MMMM(_defaultLocale.code).format(_dateTime);

  String get MMMMd => DateFormat.MMMMd(_defaultLocale.code).format(_dateTime);

  String get MMMMEEEEd => DateFormat.MMMMEEEEd(_defaultLocale.code).format(_dateTime);

  String get QQQ => DateFormat.QQQ(_defaultLocale.code).format(_dateTime);

  String get QQQQ => DateFormat.QQQQ(_defaultLocale.code).format(_dateTime);

  String get yM => DateFormat.yM(_defaultLocale.code).format(_dateTime);

  String get yMd => DateFormat.yMd(_defaultLocale.code).format(_dateTime);

  String get yMEd => DateFormat.yMEd(_defaultLocale.code).format(_dateTime);

  String get yMMM => DateFormat.yMMM(_defaultLocale.code).format(_dateTime);

  String get yMMMd => DateFormat.yMMMd(_defaultLocale.code).format(_dateTime);

  String get yMMMdjm => DateFormat.yMMMd(_defaultLocale.code).add_jm().format(_dateTime);

  String get yMMMEd => DateFormat.yMMMEd(_defaultLocale.code).format(_dateTime);

  String get yMMMEdjm => DateFormat.yMMMEd(_defaultLocale.code).add_jm().format(_dateTime);

  String get yMMMM => DateFormat.yMMMM(_defaultLocale.code).format(_dateTime);

  String get yMMMMd => DateFormat.yMMMMd(_defaultLocale.code).format(_dateTime);

  String get yMMMMdjm => DateFormat.yMMMMd(_defaultLocale.code).add_jm().format(_dateTime);

  String get yMMMMEEEEd => DateFormat.yMMMMEEEEd(_defaultLocale.code).format(_dateTime);

  String get yMMMMEEEEdjm => DateFormat.yMMMMEEEEd(_defaultLocale.code).add_jm().format(_dateTime);

  String get yQQQ => DateFormat.yQQQ(_defaultLocale.code).format(_dateTime);

  String get yQQQQ => DateFormat.yQQQQ(_defaultLocale.code).format(_dateTime);

  String get Hm => DateFormat.Hm(_defaultLocale.code).format(_dateTime);

  String get Hms => DateFormat.Hms(_defaultLocale.code).format(_dateTime);

  String get j => DateFormat.j(_defaultLocale.code).format(_dateTime);

  String get jm => DateFormat.jm(_defaultLocale.code).format(_dateTime);

  String get jms => DateFormat.jms(_defaultLocale.code).format(_dateTime);
...
//  QUERY
  ...
  static bool isAppDateTime(var input) => input is AppDateTime;
