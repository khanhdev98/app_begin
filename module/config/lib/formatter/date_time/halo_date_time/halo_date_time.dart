...
class HaloDateTime {
  ...
  HaloDateTime([var input, String? pattern]) {
    _initializeDateTime(input, pattern);
    _initializeLocale();
  }

  HaloDateTime.unixFromSecondsSinceEpoch(int timestamp) {
    _unix(timestamp * 1000);
  }

  HaloDateTime.unixFromMillisecondsSinceEpoch(int timestamp) {
    _unix(timestamp);
  }
...
//  MANIPULATE
  ...
  HaloDateTime startOf(Units units) {
    switch (units) {
  ...
  HaloDateTime endOf(Units units) {
    switch (units) {
      case Units.millisecond:
        _dateTime = DateTime(_dateTime.year, _dateTime.month, _dateTime.day, _dateTime.hour,
            _dateTime.minute, _dateTime.second, _dateTime.millisecond);
...
//  QUERY
  ...
  static bool isHaloDateTime(var input) => input is HaloDateTime;
